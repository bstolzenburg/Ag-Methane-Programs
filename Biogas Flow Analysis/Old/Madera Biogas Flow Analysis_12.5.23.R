...
file_name = paste('Madera Biogas Flow Summary_WORKING_',date,'.xlsx')                                           ####### INPUT 
  ...
  if (!file.exists(file_name)) {
    ...
    for (sheet_name in names(data_tables)) {
      ...
      if (nrow(df) > 0) {
        ...
        # If one of the dataframes is a 'gap' dataframe that has links to the 'processed_logs' sheet, write the Excel formulas for links
    ...
    # If the file already exists, ask user if they want to overwrite
  } else {
    ...
    user_input <- toupper(trimws(user_input))
    ...
    if (user_input == 'Y') {
      ...
      for (sheet_name in names(data_tables)) {
        ...
        # If dataframe has rows, remove data from existing sheet and write data
        if (nrow(df) > 0) {
      ...
      # If user wants to create a new file instead
    } else {
