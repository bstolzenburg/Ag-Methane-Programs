...
# Returning Date to character so Excel can recognize as date
processed_logs$Date <- as.character(processed_logs$Date, format = "%m/%d/%y")
processed_logs$Date <- mdy(processed_logs$Date)
...
processed_logs%>%
  summarize(Total = sum(G1_flow,flare_flow))
